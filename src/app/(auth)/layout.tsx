...
export default async function AuthLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <main className="min-h-dvh bg-[radial-gradient(circle_at_top_left,#fffdf7_0,#f1eee6_42%,#e8ded1_100%)] px-4 pb-8 pt-[calc(2rem+env(safe-area-inset-top))]">
      <div className="mx-auto grid min-h-[calc(100dvh-4rem)] w-full max-w-5xl items-center gap-8 lg:grid-cols-[1fr_420px]">
        <section className="hidden lg:block">
          ...
          <p className="mt-4 max-w-lg text-base leading-7 text-muted">
            Los usuarios registran avisos de forma sencilla. Administración revisa, asigna, adjunta facturas y cierra cada caso con trazabilidad.
          </p>
          <div className="mt-8 grid max-w-xl gap-3 sm:grid-cols-2">
            <div className="rounded-lg border border-white/70 bg-white/70 p-4 shadow-sm">
              ...
              <p className="mt-1 text-sm leading-6 text-muted">Crea una incidencia, adjunta una factura y consulta el estado.</p>
            </div>
            <div className="rounded-lg border border-white/70 bg-white/70 p-4 shadow-sm">
              <p className="text-sm font-semibold text-slate-950">Para admin</p>
              <p className="mt-1 text-sm leading-6 text-muted">Gestiona usuarios, listas, importes, estados y facturas pendientes.</p>
            </div>
          </div>
        </section>
